...
io.on('connection', function (socket) {
  ...
  socket.on('self', function (data) {
    ...
    socket.emit('message', 'To just me! (' + i + ')' + data.name + ": " + data.message)
    i++;
  });
  ...
  socket.on('all', function (data) {
    ...
    // in all rooms
    io.sockets.emit('message', 'To everyone! (' + i + ')' + data.name + ": " + data.message )
    i++;
  });
  ...
  socket.on('message', function (data) {
    ...
    io.sockets.in(socket.room).emit( data, 'To everyone in ' + socket.room + '! (' + i + ')' );
    i++;
  });
